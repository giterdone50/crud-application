...
def update(id):
	try:
        ...
        if(request.form["value1"]):
            food.name = request.form["value1"]
		if(request.form["value2"]):
            food.calories = request.form["value2"]
        if(request.form["value3"]):
            food.fat_cals = request.form["value3"]
        if(request.form["value4"]):
            food.total_fat = request.form["value4"]
        if(request.form["value5"]):
            food.saturated_fat = request.form["value5"]
        if(request.form["value6"]):
            food.trans_fat = request.form["value6"]
        if(request.form["value7"]):
            food.cholesterol = request.form["value7"]
        if(request.form["value8"]):
            food.sodium = request.form["value8"]
        if(request.form["value9"]):
            food.carbohydrates = request.form["value9"]
        if(request.form["value10"]):
            food.dietary_fiber = request.form["value10"]
        if(request.form["value11"]):
            food.sugars = request.form["value11"]
        if(request.form["value12"]):
            food.protein = request.form["value12"]
        if(request.form["value13"]):
            food.vitamin_a = request.form["value13"]
        if(request.form["value14"]):
            food.vitamin_c = request.form["value14"]
        if(request.form["value15"]):
            food.calcium = request.form["value15"]
        if(request.form["value16"]):
            food.iron = request.form["value16"]
        session.add(food)
		session.commit()
	except Exception as e:
